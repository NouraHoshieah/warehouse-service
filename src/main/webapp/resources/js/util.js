...
var getEmptyRow = function(){
	return $('<tr class="orderwarehouseService-row-feilds"></tr>');
};
...
var getwarehouseServiceCombobox = function(name, w, cls) {
	var warehouseServiceCombo = getCombo(name, w, cls);
	
	$.getJSON("warehouseService", function(result) {
        $.each(result, function(index, item) {
            warehouseServiceCombo.append('<option value="' + item.id + '">' + item.name + '</option>');
        });
    });
	
	var cont = $("<div style='width: 88%'></div>")
	cont.append(warehouseServiceCombo);
	
	warehouseServiceCombo.combobox();
...
var addWidgetsToRow = function(widgets) {
	var row = $('<tr class="orderwarehouseService-row-feilds"></tr>');
