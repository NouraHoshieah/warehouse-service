$(function() {
	$.widget( "ui.repeater", {
		...
		_create: function() {
			...
			this.mainTable = $('<table cellspacing="15" width="100%"></table>').addClass("order_warehouseService_repeater");
			...
			// Widgets
			this.itemName = getwarehouseServiceCombobox("details_01", 320, "itemName");
			...
			$('.add_ExpenseRow').on("click", function(){
			    if($('.order_warehouseService_repeater tr').length <= 6){
	    ...
	    getData: function() {
	    	var data = [];
	    	$('.order_warehouseService_repeater tr').each(function(rowIndex, row) {
	    		if (rowIndex > 0){
	    			var item = {};
...
//	    			item.quantity = $(this).find(".amount").val();
	    ...
	    getTableRow: function(){
	        var newRow = $("<tr class='orderwarehouseService-row-feilds'></tr>");
	        var td1 = getEmptyColumn();
	        td1.append(getwarehouseServiceCombobox('details_0' + this.lastChar, 320, "itemName"));
	        newRow.append(td1);
